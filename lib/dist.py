...
class Normal(nn.Module):
    ...
    def log_density(self, sample, params=None,flags=None):
...
class slab_and_spike(nn.Module):
    """Samples from a Normal distribution using the reparameterization trick.
    """

    def __init__(self, mu=0, sigma=1,alpha=0.5):
        super(slab_and_spike, self).__init__()
        self.normalization = Variable(torch.Tensor([np.log(2 * np.pi)]))

        self.mu = Variable(torch.Tensor([mu]))
        self.logsigma = Variable(torch.Tensor([math.log(sigma)]))
        self.logalpha=Variable(torch.Tensor([math.log(alpha)]))
    ...
    def _check_inputs(self, size, mu_logsigma):
        if size is None and mu_logsigma is None:
            ...
            mu = mu_logsigma.select(-1, 0)
            logsigma = mu_logsigma.select(-1, 1)
            alpha = mu_logsigma.select(-1, 2)
            return mu, logsigma, alpha
        else:
            raise ValueError(
                'Given invalid inputs: size={}, mu_logsigma={})'.format(
                    size, mu_logsigma))
    ...
    def log_density(self, sample, params=None, flags=None):
        log_eps=1e-6
        if params is not None:
            mu, logsigma, alpha = self._check_inputs(None, params)
        else:
            mu, logsigma = self._check_inputs(sample.size(), None)
            mu = mu.type_as(sample)
            logsigma = logsigma.type_as(sample)
        ...
        inv_sigma = torch.exp(-logsigma)
        tmp = (sample - mu) * inv_sigma
        arg=-0.5 * (tmp * tmp + 2 * logsigma + c)
        term1 = torch.log(alpha + log_eps) + arg
        term2 = torch.log(1. - alpha + log_eps) + torch.log(torch.tensor(flags.delta_value))
        output= term1 * 1.*(torch.abs(sample) >= flags.delta_threshold) + \
                term2 * 1.*(torch.abs(sample) < flags.delta_threshold)
        return output
